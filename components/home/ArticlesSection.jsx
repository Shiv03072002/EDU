...
export default function ArticlesSection() {
  ...
  const prev = () => setCurrentSlide((s) => (s - 1 + articles.length) % articles.length);
  const next = () => setCurrentSlide((s) => (s + 1) % articles.length);

  return (
    <section className="relative py-12 lg:py-20 bg-white overflow-hidden">

      {/* Decorative triangle top-right of heading area */}
      <div className="hidden lg:block absolute top-16 right-[44%] w-0 h-0
        border-l-[10px] border-l-transparent
        border-r-[10px] border-r-transparent
        border-b-[16px] border-b-gray-300 opacity-60"
      />

      {/* 3x2 dots under the triangle */}
      <div className="hidden lg:grid absolute top-28 right-[42%] grid-cols-3 gap-3 opacity-40">
        {[...Array(6)].map((_, i) => (
      ...
      <div className="hidden lg:grid absolute right-2 top-[70%] grid-cols-3 gap-4">
        {[...Array(9)].map((_, i) => (
          <div key={i} className="w-1.5 h-1.5 bg-green-500 rounded-full" />
        ))}
      </div>
      <div className="hidden lg:block absolute right-16 top-[73%] w-0 h-0
      ...
      <div className="hidden lg:block absolute bottom-16 right-56 w-72 h-40">
        <img src="/images/articles/hand.png" alt="" className="w-full h-full object-cover" />
      </div>
      <div className="hidden lg:block absolute bottom-0 right-0 w-80 h-44">
        <img src="/images/articles/element.png" alt="" className="w-full h-full" />
      </div>
      ...
      <div className="px-4 sm:px-6 lg:px-16 max-w-[1400px] mx-auto">
        ...
        <div className="hidden lg:block">
          ...
          <div className="flex items-start mb-10">
            ...
            <div style={{ width: "45%" }} className="pt-2 pl-8">
          ...
          <div className="flex items-start gap-0">
            ...
            {/* RIGHT — Latest News */}
            <div style={{ width: "45%" }} className="pl-8 border-l border-gray-100">
              <h3 className="text-xl font-bold mb-6 text-gray-900">Latest News</h3>
                ...
                {news.map((item, i) => (
                  <div key={i} className="flex gap-4 items-start group cursor-pointer">
                    ...
                    <div
                      ...
                      style={{ ...checkerStyle(10), width: "100px", height: "70px" }}
                    />
                    <div className="min-w-0">
                      ...
                      <p className="text-xl font-bold text-gray-900 group-hover:text-blue-600 transition-colors leading-snug">
                        {item.title}
                      </p>
                    </div>
                  </div>
        ...
        <div className="hidden md:block lg:hidden">
          ...
          <div className="grid grid-cols-2 gap-5 mb-10">
            {articles.map((item, i) => (
              <div key={i} className="group cursor-pointer">
                <div className="w-full h-44 rounded-xl" style={checkerStyle(16)} />
                <div className="mt-3 text-xs text-gray-400 flex items-center gap-2">
                  <span className="font-semibold text-gray-500 uppercase">{item.tag}</span>
          ...
          <div className="space-y-4">
            {news.map((item, i) => (
              <div key={i} className="flex gap-4 items-start group cursor-pointer">
                <div className="w-24 h-16 rounded-lg flex-shrink-0" style={checkerStyle(10)} />
                <div>
        ...
        <div className="block md:hidden">
          ...
          <div>
            <div className="relative rounded-xl overflow-hidden">
              <div className="w-full h-48" style={checkerStyle(16)} />
              <button
                onClick={prev}
                className="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white shadow-md flex items-center justify-center text-gray-700"
              >
                <ChevronLeft className="w-4 h-4" />
              </button>
              <button
                onClick={next}
                className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white shadow-md flex items-center justify-center text-gray-700"
              >
                <ChevronRight className="w-4 h-4" />
              </button>
            </div>
            <div className="mt-3 text-xs text-gray-400 flex items-center gap-2">
              <span className="font-semibold text-gray-500 uppercase">{articles[currentSlide].tag}</span>
              <span className="text-gray-300">•</span>
              ...
              {articles.map((_, i) => (
                <button
                  ...
                  className={`rounded-full transition-all duration-300 ${
                    i === currentSlide ? "w-4 h-2 bg-blue-600" : "w-2 h-2 bg-gray-300"
                  }`}
                />
              ))}
            </div>
            ...
            <div className="flex justify-center gap-2 mt-4">
              {news.map((_, i) => (
                <button
                  ...
                  className={`rounded-full transition-all duration-300 ${
                    i === currentNews ? "w-4 h-2 bg-blue-600" : "w-2 h-2 bg-gray-300"
                  }`}
                />
              ))}
            </div>
